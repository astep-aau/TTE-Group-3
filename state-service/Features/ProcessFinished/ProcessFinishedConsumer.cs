...
{
    public class ProcessFinishedConsumer : BackgroundService
    {
        private readonly IRabbitMqConnection _connection;
        private readonly IServiceProvider _sp;
        private readonly ILogger<ProcessFinishedConsumer> _logger;
        private IModel? _channel;
        private const string QueueName = "process-finished";

        public ProcessFinishedConsumer(IRabbitMqConnection connection, IServiceProvider sp, ILogger<ProcessFinishedConsumer> logger)
        {
        ...
        protected override Task ExecuteAsync(CancellationToken stoppingToken)
        {
            ...
            consumer.Received += async (_, ea) =>
            {
                ...
                using var activity = Infrastructure.Observability.ActivitySourceHolder.Source.StartActivity("consume.process-finished");
                try
                {
                    var json = Encoding.UTF8.GetString(ea.Body.ToArray());
                    var msg = JsonSerializer.Deserialize<ProcessFinishedMessage>(json);
                    if (msg is null)
                    {
                    ...
                    var validator = scope.ServiceProvider.GetRequiredService<IValidator<ProcessFinishedMessage>>();
                    ...
                    var handler = scope.ServiceProvider.GetRequiredService<ProcessFinishedHandler>();
