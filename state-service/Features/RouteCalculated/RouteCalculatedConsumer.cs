...
{
    public class RouteCalculatedConsumer : BackgroundService
    {
        private readonly IRabbitMqConnection _connection;
        private readonly IServiceProvider _sp;
        private readonly ILogger<RouteCalculatedConsumer> _logger;
        private IModel? _channel;
        private const string QueueName = "route-calculated";

        public RouteCalculatedConsumer(IRabbitMqConnection connection, IServiceProvider sp, ILogger<RouteCalculatedConsumer> logger)
        {
        ...
        protected override Task ExecuteAsync(CancellationToken stoppingToken)
        {
            ...
            consumer.Received += async (_, ea) =>
            {
                ...
                using var activity = Infrastructure.Observability.ActivitySourceHolder.Source.StartActivity("consume.route-calculated");
                try
                {
                    var json = Encoding.UTF8.GetString(ea.Body.ToArray());
                    var msg = JsonSerializer.Deserialize<RouteCalculatedMessage>(json);
                    if (msg is null)
                    {
                    ...
                    var validator = scope.ServiceProvider.GetRequiredService<IValidator<RouteCalculatedMessage>>();
                    ...
                    var handler = scope.ServiceProvider.GetRequiredService<RouteCalculatedHandler>();
