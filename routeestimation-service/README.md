...
```json
{
  "5030596352": {
    "Lat": 45.782134,
    "Lon": 126.557067,
    "OutwardEdges": [7919, 20623],
    "OutwardVertices": ["334305011", "334304113"],
    "BackwardEdges": [14224],
    "BackwardVertices": ["334304998"]
  }
}
```
...
| Field             | Type       | Notes                                         |
|-------------------|------------|-----------------------------------------------|
| Lat               | double     | Node latitude in WGS84                        |
| Lon               | double     | Node longitude in WGS84                       |
| OutwardEdges      | int[]      | Edge IDs originating from this node           |
| OutwardVertices   | string[]   | Node IDs at the end of outward edges          |
| BackwardEdges     | int[]      | Edge IDs terminating at this node             |
| BackwardVertices  | string[]   | Node IDs at the start of backward edges       |
...
| Field     | Type    | Notes                                       |
|-----------|---------|---------------------------------------------|
| LengthCm  | double  | Edge length in centimeters                  |
